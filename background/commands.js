...
async function onCommand(name, currentTab) {
  ...
  // Disables hotkeys when a remote service is on. You can add more to the list if you want
  chrome.tabs.query({ windowType: 'normal' }, function (tabs) {
    let isTabConnection = false
    tabs.forEach(tab => {
      if (tab.active == true) {
        if (tab.url.includes("https://remotedesktop.google.com/access/session/")) {
