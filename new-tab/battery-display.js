class BatteryDisplay {
  ...
  async getBattery() {
    let battery = await navigator.getBattery().catch(() => reportError("Error reading battery."));
  ...
  async render() {
    ...
    this.element.textContent = battery.isFull ? "Battery full" : [this.getPercentMessage(battery), this.getChargingMessage(battery), this.getTimeMessage(battery)].join(" ~ ");
  }
